from telegram import Update, ReplyKeyboardMarkup
from telegram.ext import ContextTypes, ConversationHandler, MessageHandler, filters

SEASON, PLANT_TYPE = range(2)

SEASON_KEYBOARD = [
    ["🌱 Весна", "☀️ Лето"],
    ["🍂 Осень", "❄️ Зима"]
]

SEASONAL_RECOMMENDATIONS = {
    "весна": {
        "title": "🌱 *ВЕСЕННИЙ УХОД ЗА РАСТЕНИЯМИ*",
        "recommendations": """
*💧 ПОЛИВ:*
• Постепенно увеличивайте полив
• Начинайте регулярные подкормки
• Следите за пробуждением растений

*🌿 ОСНОВНЫЕ ЗАБОТЫ:*
• Пересадка в новые горшки
• Обрезка и формирование кроны
• Начало вегетационного периода
• Профилактика от вредителей

*⚠️ ОСОБОЕ ВНИМАНИЕ:*
• Постепенное приучение к свету
• Защита от весенних сквозняков
• Контроль за молодыми побегами
"""
    },
    "лето": {
        "title": "☀️ *ЛЕТНИЙ УХОД ЗА РАСТЕНИЯМИ*",
        "recommendations": """
*💧 ПОЛИВ:*
• Увеличьте частоту полива
• Регулярно опрыскивайте листья
• Следите за пересыханием почвы

*🌿 ОСНОВНЫЕ ЗАБОТЫ:*
• Защита от палящего солнца
• Проветривание помещений
• Борьба с перегревом
• Регулярные подкормки

*⚠️ ОСОБОЕ ВНИМАНИЕ:*
• Притенение на южных окнах
• Контроль влажности воздуха
• Защита от кондиционеров
• Возможность выноса на балкон
"""
    },
    "осень": {
        "title": "🍂 *ОСЕННИЙ УХОД ЗА РАСТЕНИЯМИ*",
        "recommendations": """
*💧 ПОЛИВ:*
• Постепенно сокращайте полив
• Уменьшайте подкормки
• Готовьте к периоду покоя

*🌿 ОСНОВНЫЕ ЗАБОТЫ:*
• Подготовка к зимовке
• Санитарная обрезка
• Последние пересадки
• Уборка опавших листьев

*⚠️ ОСОБОЕ ВНИМАНИЕ:*
• Защита от сквозняков
• Сокращение светового дня
• Понижение температуры
• Профилактика грибковых болезней
"""
    },
    "зима": {
        "title": "❄️ *ЗИМНИЙ УХОД ЗА РАСТЕНИЯМИ*",
        "recommendations": """
*💧 ПОЛИВ:*
• Минимальный полив
• Давайте почве хорошо просохнуть
• Используйте теплую воду

*🌿 ОСНОВНЫЕ ЗАБОТЫ:*
• Период покоя для большинства растений
• Защита от холодных окон
• Борьба с сухим воздухом
• Досвечивание фитолампами

*⚠️ ОСОБОЕ ВНИМАНИЕ:*
• Защита от батарей отопления
• Контроль температуры у окон
• Избегайте холодных сквозняков
• Минимальные подкормки
"""
    }
}


async def start_seasonal_recommendations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Начало диалога выбора сезона"""
    await update.message.reply_text(
        "🌿 *Выберите текущее время года:*",
        parse_mode='Markdown',
        reply_markup=ReplyKeyboardMarkup(SEASON_KEYBOARD, one_time_keyboard=True, resize_keyboard=True)
    )
    return SEASON


async def handle_season_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка выбора сезона"""
    user_input = update.message.text.lower()

    season_map = {
        "🌱 весна": "весна",
        "весна": "весна",
        "☀️ лето": "лето",
        "лето": "лето",
        "🍂 осень": "осень",
        "осень": "осень",
        "❄️ зима": "зима",
        "зима": "зима"
    }

    season = season_map.get(user_input)

    if not season:
        await update.message.reply_text(
            "❌ Пожалуйста, выберите время года из предложенных вариантов:",
            reply_markup=ReplyKeyboardMarkup(SEASON_KEYBOARD, one_time_keyboard=True, resize_keyboard=True)
        )
        return SEASON

    context.user_data['season'] = season
    recommendations = SEASONAL_RECOMMENDATIONS[season]

    response = f"{recommendations['title']}\n\n{recommendations['recommendations']}"

    await update.message.reply_text(
        response,
        parse_mode='Markdown',
        reply_markup=ReplyKeyboardMarkup([["⬅️ Назад"]], resize_keyboard=True)
    )

    return ConversationHandler.END


async def cancel_recommendations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отмена диалога"""
    await update.message.reply_text(
        "Возвращаюсь в главное меню!",
        reply_markup=ReplyKeyboardMarkup([
            ["🌱 Мои растения", "🔍 Диагностика"],
            ["📚 Рекомендации", "🌍 Поиск растений"],
            ["👨‍🌾 Чат с агрономом"]
        ], resize_keyboard=True)
    )
    return ConversationHandler.END

def build_recommendations_conversation():
    return ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^📚 Рекомендации$"), start_seasonal_recommendations)],
        states={
            SEASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_season_choice)],
        },
        fallbacks=[MessageHandler(filters.Regex("^⬅️ Назад$"), cancel_recommendations)],
        map_to_parent={
            ConversationHandler.END: ConversationHandler.END
        }
    )


async def get_recommendations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Старая функция - перенаправляем в новый диалог"""
    await start_seasonal_recommendations(update, context)